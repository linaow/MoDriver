...
class DCDriver():
    def __init__(self, datasets, model_path='./DCDriver.h5', epochs=30, batch_size=16):
        self.latent_dim = 128
        self.n = len(datasets)
        self.epochs = epochs
        ...
        for i in range(self.n):
            ...
            #self.latent_dim += datasets[i].shape[1]
...
def file2data(cancer_type, train_pos, train_neg):
    ...
    for mode in mode_all:
        ...
        #fea_sim_one = './sim/%s/%s.fea' % ('Pancan', mode)
        # fea_sim2_one = './sim2/%s/%s.fea' % (cancer_type, mode)
        df_sim_one = pd.read_csv(fea_sim_one, header=0, index_col=0, sep='\t')
        # df_sim2_one = pd.read_csv(fea_sim2_one, header=0, index_col=0, sep='\t')
        ids = list(df_one.index)
        # pos_ids = list(set(pos_ids) & set(ids))
        # neg_ids = list(set(neg_ids) & set(ids))
        ...
        # sim_mat2 = df_sim2_one.values.astype(float)
...
def fit_cv(Xs, y, k=10, method='RF', b_plot=False):
    n = Xs[0].shape[0]
    ...
    for i in range(1, k + 1):
        ...
        if method == 'SVM':
            ...
            probas_ = model.predict_proba(X_all_test)[:, 1]
            del model
        elif method == 'RF':
            model = RandomForestClassifier(n_estimators=1000, max_depth=3, random_state=0)
            model.fit(X_all_train, y_train)
...
def predict(Xs, type, method='RF', b_null= False):
    model_path = './model/%s.model' % type
    X = []
    for j in range(len(Xs)):
        scaler_path = './model/%s_%d.scaler' % (type, j)
        # if b_null:
        #     scaler_path = './model/Pancan_%d.scaler' % (j)
...
def eval(Xs, y, type, method='RF'):
    ...
    if method != 'MODNN':
        model = pickle.load(open(model_path, 'rb'))
        X_all = np.concatenate(X, axis=1)
        y_p = model.predict_proba(X_all)[:, 1]
    else:
        dcd = DCDriver(X)
        dcd.model = load_model(model_path)
...
def main(argv=sys.argv):
    ...
    if args.mode == 'train':
        ...
        ban_list = ['::TTN::','::DST::', '::DMD::', '::CACNA1E::', '::COL11A1::', '::mitranscriptome::']
        ids_new =[]
        for id in ids:
            b_keep = True
            for ban in ban_list:
    ...
    if args.mode == 'neg':
        apps = ['2020plus', 'ActiveDriver', 'CompositeDriver', 'MuSiC', 'MutSig2CV',
                'OncodriveCLUST', 'OncodriveFML', 'e-Driver']
        nb_line = 0
        for app in apps:
            nb_line += 1
            thr = 0.6
            path = '../coding/%s/PANCAN.txt' % app
            df = pd.read_csv(path, header=0, sep='\t', index_col=0, usecols=['gene', 'qvalue'])
            df = df[df['qvalue'] > thr]
            if nb_line == 1:
        ...
        df.to_csv(random_out_file, header=True, index=False, sep='\t', compression='gzip', float_format='%.3f')
